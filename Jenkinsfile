  ...
  stages {
    ...
    stage('Run Open Source tests') {
      parallel {
        stage("Test conjur_variable lookup plugin") {
          steps {
            sh './ci/test.sh -b ${GIT_BRANCH} -d conjur_variable'
        ...
        stage("Test conjur_host_identity role") {
          steps {
            sh './ci/test.sh -b ${GIT_BRANCH} -d conjur_host_identity'
    ...
    stage('Run Enterprise tests') {
      stages {
        stage("Test conjur_variable lookup plugin") {
          steps {
            sh './ci/test.sh -e -b ${GIT_BRANCH} -d conjur_variable'
            junit 'roles/conjur_variable/junit/*'
          }
        }

        stage("Test conjur_host_identity role") {
          steps {
            sh './ci/test.sh -e -b ${GIT_BRANCH} -d conjur_host_identity'
            junit 'roles/conjur_host_identity/tests/junit/*'
          }
        }
      }
